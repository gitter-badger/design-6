...
object PortalUtils {
  ...
  private[components] def getClosableMods(isClosableOpt: Option[IsClosable], close: Callback): TagMod = {
  ...
  private[components] def detach(renderContent: Callback => VdomElement): Callback = Callback {

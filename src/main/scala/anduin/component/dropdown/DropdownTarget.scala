...
private[dropdown] class DropdownTarget[A] {
  ...
  private def render(props: Props): VdomElement = {
    val styles = ButtonStyle.getStyles(
      ...
      style = DropdownTarget.getButtonStyle(props.outer.style),
      isSelected = props.downshift.isOpen.contains(true),
      isFullWidth = props.outer.isFullWidth
    )
    <.button(
      ^.tpe := "button",
      ^.disabled := props.outer.isDisabled || props.outer.options.isEmpty,
      Util.getModsFromProps(props.downshift.getToggleButtonProps()),
      styles
    )(
      props.outer.value.fold(props.outer.placeholder)(props.outer.renderValue),
      DropdownTarget.icon
    )
  }
